...
class TokenClassification(SupervisedObjective):
    ...
    def _wordpiece_token_label_alignment(
        self, texts: Iterable[str], labels: Iterable[str]
    ) -> Iterable[Dict[str, torch.LongTensor]]:
        """
    ...
    def _get_inputs_iterator(
        ...
        aligned_collated_iter = self._wordpiece_token_label_alignment(
            texts_iter, labels_iter
        )
    ...
    def _compute_loss(
        ...
        if attention_mask is not None:
            ...
            loss = loss_fct(
                logit_outputs.view(-1, len(self.labels_map)), labels.view(-1)
            )
...
class SequenceClassification(SupervisedObjective):
    ...
    def _compute_loss(
        ...
        if labels.dim() == 1:
            ...
            loss = loss_fn(
                logit_outputs.view(-1, logit_outputs.shape[-1]), labels.view(-1)
            )
        else:
