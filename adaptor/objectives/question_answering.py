        ...
        for src_text, text_pair, label in zip(
            *self._per_split_iterators_text_pair(split)
            ...
            tokenized_label = self.tokenizer(
                label, max_length=MAX_LENGTH, truncation=True, padding="max_length"
            )
            label_wo_padding = self.tokenizer(label)
            # find indexes for answer:
            if set(tokenized_label["input_ids"][1:-1]).issubset(
                set(out_sample["input_ids"])
            ):
                start_position = out_sample["input_ids"].index(
                    tokenized_label["input_ids"][1:-1][0]
                )
